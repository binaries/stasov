...
STRING :
    ('"' ([a-zA-Z0-9]|'_'|' ')* '"') {setText(getText().substring(1, getText().length()-1));} // strip the quotes;
    | ('{' ([a-zA-Z0-9]|'_'|' ')* '}') {setText(getText().substring(1, getText().length()-1));} // strip the curly brackets;
;

package com.pocketmath.stasov.pmtl;

/**
 * Created by etucker on 6/9/15.
 */
public class PocketTLLanguageException extends Exception {
    public PocketTLLanguageException() {
        super();
    }

    public PocketTLLanguageException(Throwable cause) {
        super(cause);
    }

    public PocketTLLanguageException(String message) {
        super(message);
    }

    public PocketTLLanguageException(String message, Throwable cause) {
        super(message, cause);
    }

    protected PocketTLLanguageException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
        super(message, cause, enableSuppression, writableStackTrace);
    }
}
